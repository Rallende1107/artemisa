"""Formularios del panel para `movie` — UNO POR MODELO, AUTÓNOMOS.
...
Sí, hay repetición entre formularios parecidos, y con los de `serie` (son gemelos).
Es a propósito: se lee y se cambia uno sin tocar los demás.

`MovieLog` no tiene formulario a propósito: es un REGISTRO, no se escribe a mano.
"""
...
class GenreForm(FormBaseCategory):
    """Género de película. `explicit` marca los géneros +18 (filtran el catálogo público)."""

    class Meta:
        model = Genre
        fields = ['name', 'name_esp', 'description', 'explicit', 'image', 'is_active']

    duplicate_message = _('Ya existe un género de película con este nombre.')
...
class MovieCastForm(forms.ModelForm):
    """Reparto: una persona interpreta un personaje en una película. El modelo exige
    que (película, persona, rol, personaje) sea único: se comprueba en `clean()`."""

    class Meta:
        model = MovieCast
        fields = ['movie', 'person', 'role', 'character_name', 'is_active']

    movie = forms.ModelChoiceField(
        label=_('Película'),
        help_text=_('Película a la que pertenece este reparto. Escribe para buscar.'),
        required=True, queryset=Movie.objects.all(), empty_label=_('—'),
        widget=AjaxSelect(url_name='panel:movie_select', attrs={
            'aria-label': _('Película'), 'required': 'required',
            'title': _('Película a la que pertenece este reparto. Escribe para buscar.'),
        }),
    ...
    role = forms.ModelChoiceField(
        ...
        widget=AjaxSelect(url_name='panel:movie-role_select', attrs={
    ...
    def clean(self):
        """`unique_together = (movie, person, role, character_name)` en el modelo."""
        cleaned = super().clean()
        movie = cleaned.get('movie')
        person = cleaned.get('person')
        if movie and person:
            duplicados = MovieCast.objects.filter(
                movie=movie, person=person, role=cleaned.get('role'),
                character_name=cleaned.get('character_name') or '',
            )
            if self.instance.pk:
                duplicados = duplicados.exclude(pk=self.instance.pk)
            if duplicados.exists():
                self.add_error(None, 'Ya existe esta misma línea de reparto para la película.')
        return cleaned


class MovieForm(forms.ModelForm):
    """La película en sí. Los desplegables NO vuelcan sus tablas: buscan en el servidor
    ...
    class Meta:
        ...
        fields = ['title', 'title_secundary', 'release_year', 'duration_minutes',
                  'synopsis', 'movie_type', 'movie_rating', 'genres', 'producers',
    ...
    release_year = forms.IntegerField(
        ...
        widget=forms.NumberInput(attrs={
            'placeholder': _('1999'), 'aria-label': _('Año'), 'autocomplete': 'off',
    ...
    duration_minutes = forms.IntegerField(
        ...
        help_text=_('Duración de la película, en minutos (opcional).'),
        required=False, min_value=0,
        widget=forms.NumberInput(attrs={
            'placeholder': _('136'), 'aria-label': _('Duración (min)'), 'autocomplete': 'off',
            'inputmode': 'numeric', 'step': 1, 'min': 0,
            'title': _('Duración de la película, en minutos (opcional).'),
        }),
        error_messages={'invalid': _('Introduce un número entero.'),
    ...
    movie_type = forms.ModelChoiceField(
        label=_('Tipo'),
        help_text=_('Largometraje, cortometraje, documental… Escribe para buscar.'),
        required=False, queryset=Type.objects.all(), empty_label=_('—'),
        widget=AjaxSelect(url_name='panel:movie-type_select', attrs={
            'aria-label': _('Tipo'),
            'title': _('Largometraje, cortometraje, documental… Escribe para buscar.'),
        }),
        error_messages={'invalid_choice': _('Selecciona una opción válida.')},
    )
    movie_rating = forms.ModelChoiceField(
        label=_('Clasificación'),
        help_text=_('Clasificación por edad (PG-13, R…). Escribe para buscar.'),
        required=False, queryset=Rating.objects.all(), empty_label=_('—'),
        widget=AjaxSelect(url_name='panel:movie-rating_select', attrs={
            'aria-label': _('Clasificación'),
            'title': _('Clasificación por edad (PG-13, R…). Escribe para buscar.'),
        }),
        error_messages={'invalid_choice': _('Selecciona una opción válida.')},
    )
    genres = forms.ModelMultipleChoiceField(
        ...
        widget=AjaxSelectMultiple(url_name='panel:movie-genre_select', attrs={
    ...
    def clean(self):
        """Duplicados por TÍTULO + AÑO (el slug único lleva el año): «The Thing»
        de 1982 y la de 2020 CONVIVEN; choca solo mismo título y mismo año.
        ...
        candidatos.discard('')
        if candidatos:
            duplicadas = Movie.objects.filter(slug__in=candidatos)
            if self.instance.pk:
                duplicadas = duplicadas.exclude(pk=self.instance.pk)
            if duplicadas.exists():
                raise forms.ValidationError('Ya existe una película con este título y ese año.')
        return data
...
class MovieImageForm(forms.ModelForm):
    ...
    class Meta:
        model = MovieImage
        fields = ['movie', 'order', 'image', 'image_url', 'is_active']

    movie = forms.ModelChoiceField(
        label=_('Película'),
        help_text=_('Película a la que pertenece la imagen. Escribe para buscar.'),
        required=True, queryset=Movie.objects.all(), empty_label=_('—'),
        widget=AjaxSelect(url_name='panel:movie_select', attrs={
            'aria-label': _('Película'), 'required': 'required',
            'title': _('Película a la que pertenece la imagen. Escribe para buscar.'),
        }),
        error_messages={'required': _('Este campo es obligatorio.'),
                        'invalid_choice': _('Selecciona una opción válida.')},
    )
...
class MovieRelationForm(forms.ModelForm):
    """Relación entre dos películas (secuela, precuela, spin-off…), en un sentido."""

    class Meta:
        model = MovieRelation
        fields = ['movie', 'related', 'relation_type', 'is_active']

    movie = forms.ModelChoiceField(
        label=_('Película'), help_text=_('La película de la que sale la relación. Escribe para buscar.'),
        required=True, queryset=Movie.objects.all(), empty_label=_('—'),
        widget=AjaxSelect(url_name='panel:movie_select', attrs={'aria-label': _('Película'), 'required': 'required'}),
    ...
    related = forms.ModelChoiceField(
        label=_('Relacionada'), help_text=_('La otra película. Escribe para buscar.'),
        required=True, queryset=Movie.objects.all(), empty_label=_('—'),
        widget=AjaxSelect(url_name='panel:movie_select', attrs={'aria-label': _('Relacionada'), 'required': 'required'}),
        error_messages={'required': _('Este campo es obligatorio.'), 'invalid_choice': _('Selecciona una opción válida.')},
    )
    ...
    def clean(self):
        cleaned = super().clean()
        movie, related = cleaned.get('movie'), cleaned.get('related')
        if movie and related and movie == related:
            self.add_error('related', _('Una película no puede relacionarse consigo misma.'))
        if movie and related:
            duplicados = MovieRelation.objects.filter(movie=movie, related=related, relation_type=cleaned.get('relation_type'))
            if self.instance.pk:
                duplicados = duplicados.exclude(pk=self.instance.pk)
            if duplicados.exists():
                self.add_error(None, _('Ya existe esta relación entre las dos películas.'))
        return cleaned


class MovieStaffForm(forms.ModelForm):
    """Equipo técnico: una persona con un rol de producción en una película. El modelo
    exige que (película, persona, rol) sea único: se comprueba en `clean()`."""
    ...
    movie = forms.ModelChoiceField(
        label=_('Película'),
        help_text=_('Película en la que trabajó. Escribe para buscar.'),
        required=True, queryset=Movie.objects.all(), empty_label=_('—'),
        widget=AjaxSelect(url_name='panel:movie_select', attrs={
            'aria-label': _('Película'), 'required': 'required',
            'title': _('Película en la que trabajó. Escribe para buscar.'),
        }),
        error_messages={'required': _('Este campo es obligatorio.'),
                        'invalid_choice': _('Selecciona una opción válida.')},
    )
    ...
    def clean(self):
        """`unique_together = (movie, person, role)` en el modelo."""
        cleaned = super().clean()
        movie = cleaned.get('movie')
        person = cleaned.get('person')
        if movie and person:
            duplicados = MovieStaff.objects.filter(
                movie=movie, person=person, role=cleaned.get('role'))
            if self.instance.pk:
                duplicados = duplicados.exclude(pk=self.instance.pk)
            if duplicados.exists():
                self.add_error(None, 'Esta persona ya está registrada con ese rol en la película.')
        return cleaned


class MovieTitleForm(forms.ModelForm):
    """Título alternativo de una película, por idioma. El modelo exige que
    (película, idioma, título) sea único: se comprueba en `clean()`."""
    ...
    movie = forms.ModelChoiceField(
        label=_('Película'),
        help_text=_('Película a la que pertenece el título. Escribe para buscar.'),
        required=True, queryset=Movie.objects.all(), empty_label=_('—'),
        widget=AjaxSelect(url_name='panel:movie_select', attrs={
            'aria-label': _('Película'), 'required': 'required',
            'title': _('Película a la que pertenece el título. Escribe para buscar.'),
        }),
        error_messages={'required': _('Este campo es obligatorio.'),
                        'invalid_choice': _('Selecciona una opción válida.')},
    )
    ...
    def clean(self):
        """`unique_together = (movie, title_lang, title)` en el modelo."""
        cleaned = super().clean()
        movie = cleaned.get('movie')
        title_lang = cleaned.get('title_lang')
        title = cleaned.get('title')
        if movie and title_lang and title:
            duplicados = MovieTitle.objects.filter(movie=movie, title_lang=title_lang, title=title)
            if self.instance.pk:
                duplicados = duplicados.exclude(pk=self.instance.pk)
            if duplicados.exists():
                self.add_error('title', 'Esta película ya tiene ese título en ese idioma.')
        return cleaned


class RatingForm(FormBaseCategory):
    """Clasificación por edad de una película, con su acrónimo (PG-13, R…)."""
    ...
    acronym = forms.CharField(
        label=_('Acrónimo'),
        help_text=_('Sigla de la clasificación (PG-13, R…). Se guarda en MAYÚSCULAS.'),
        required=False, max_length=15, strip=True,
        widget=forms.TextInput(attrs={
            'placeholder': _('PG-13'), 'aria-label': _('Acrónimo'), 'autocomplete': 'off',
            'maxlength': 15, 'style': 'text-transform:uppercase',
            'title': _('Sigla de la clasificación (PG-13, R…). Se guarda en MAYÚSCULAS.'),
        }),
...
class RoleForm(forms.ModelForm):
    """Rol en una película (Protagonista, Director, Guionista…), agrupado por tipo de rol."""
    ...
    def clean_name(self):
        ...
        if slug:
            ...
            if self.instance.pk:
                duplicados = duplicados.exclude(pk=self.instance.pk)
            if duplicados.exists():
                raise forms.ValidationError('Ya existe un rol de película con este nombre.')
        return name
...
class TypeForm(FormBaseCategory):
    """Tipo de película: Largometraje, Cortometraje, Documental…"""
...
class MovieLogForm(forms.ModelForm):
    """Log de películas: nivel, proceso, mensaje y activo. El Create existe por la regla 1:1:N (lo normal es que lo escriba el sistema)."""
...
class MovieImageDownloadForm(DescargarImagenesForm):
    TIPO = 'movies.MovieImage'
    success_url = 'panel:movie-image_list'


class MovieImageRetryForm(ReintentarMuertasForm):
    TIPO = 'movies.MovieImage'
    success_url = 'panel:movie-image_download'
