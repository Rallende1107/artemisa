"""people · DESCARGAR IMÁGENES: una vista por tabla de imágenes, como «Procesar»: cuántas faltan, N o todas, en el worker
(se ve en Tareas y en Dozzle), por lotes y cancelable. Los fallos quedan en el log de la app."""
from django.utils.translation import gettext_lazy as _

from apps.people import forms
from apps.people.views.base import BasePersonImage
from apps.system import tasks as system_tasks
from core.shared.views.base import BaseImport


class PersonImageDownloadView(BasePersonImage, BaseImport):
    """Descargar a disco las imágenes pendientes de personas."""
    forms = (forms.PersonImageDownloadForm, forms.PersonImageRetryForm)
    source_key = "imágenes"
    source_label = _("Imágenes")
    task_procesar = system_tasks.download_images_batch_task
    title = _("Descargar imágenes de personas")
